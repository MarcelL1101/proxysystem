    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (args.length >= 1) {
            ...
            for(final ProxiedPlayer team : ProxySystem.getInstance().getProxy().getPlayers()) {
                if (team.hasPermission("proxysystem.adminchat")) {
                    team.sendMessage(ProxySystem.getInstance().configManager.getMessageAdminChat(player.getName(), message));
                }
            }
        } else {
            String messageKey = "onUse";
            String localizedMessage = ProxySystem.getInstance().langeLanguageManager.getLocalizedMessageOnUseArguments(player.getUniqueId(), messageKey, "adminchat", "Message");
            player.sendMessage(localizedMessage);
        }
    }
